...
- A voice picker for Bud's spoken suggestions, with a one-click preview button.

### Changed

- Redesigned the web UI into a single-screen "Field Guide" theme built around Bud, the owl mascot.
- The post-session review is now generated by OpenAI via the `/api/review` route instead of a local recap.
- Coach suggestions are now a single sentence capped around 15 words, and the coaching screen was simplified (removed the phase chip and the spoken-suggestions status indicator).
